...
const MasterBukuPage = () => {
	...
	const handleEdit = (id) => {
		axiosInstance.get(`/MasterBuku/${id}`).then((res) => {
	...
	const handleDelete = (id) => {
		axiosInstance.delete(`/MasterBuku/${id}`).then((res) => {
	...
	return (
		<Container
			...
			<Flex align={"center"}>
				<Box>
					<Text textStyle={"title-big"}>Master Buku</Text>
				</Box>
				<Spacer />
				<Box>
					<Button
				...
				<Table variant="simple" borderWidth={2} borderColor={"main"}>
					<Thead borderWidth={2} background={"main"} borderColor={"main"}>
						<Tr>
							...
							<Th color={"white"}>Pengarang</Th>
							<Th color={"white"}>Penerbit</Th>
							<Th color={"white"}>Tahun Terbit</Th>
								...
								{data?.data?.map((val) => (
									<Tr borderWidth={2} borderColor={"main"}>
										...
										<Td>
											<HStack spacing={2}>
												<IconButtonEdit
													onClick={() => handleEdit(val.idbuku)}
												/>
												<IconButtonDelete
													onClick={() => handleDelete(val.idbuku)}
												/>
											</HStack>
										</Td>
									</Tr>
